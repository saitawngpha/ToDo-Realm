...
//  Realm ToDo-2
//
//  Created by Stewart Lynch on 2022-03-04.
//
...
struct CountryInfo: Decodable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {
        case name, emoji, code
    }
    
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        name = try container.decode(String.self, forKey: .name)
        flag = try container.decode(String.self, forKey: .emoji)
        countryCode = try container.decode(String.self, forKey: .code)
    }
    
    static var array: [CountryInfo] {
        let decoder = JSONDecoder()
        return try! decoder.decode([CountryInfo].self, from: Data(Self.jsonString.utf8)).sorted(by: {$0.name < $1.name})
    }
    
    // Source: https://cdn.jsdelivr.net/npm/country-flag-emoji-json@2.0.0/dist/index.json
    // I want this to be completely offline and not dependent on a wired connection so thus, I have
    // broght this json in to here rather than fetching it each time.
